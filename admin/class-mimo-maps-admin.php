 ...
 * functionality, then refer to `class-mimo-maps.php`
 *
 * 
 *
 * @package Mimo_Maps_Admin
 * @author  Mimo <mail@mimo.media>
 */
class Mimo_Maps_Admin {
	...
	private function __construct() {
		 ...
		 * Add metabox
		 */
		add_action( 'cmb2_init', array( $this, 'mimo_maps_metaboxes' ) );
	...
	public function enqueue_admin_styles() {
		...
		if ( $this->plugin_screen_hook_suffix == $screen->id || strpos( $_SERVER[ 'REQUEST_URI' ], 'index.php' ) || strpos( $_SERVER[ 'REQUEST_URI' ], get_bloginfo( 'wpurl' ) . '/wp-admin/' ) ) {
			wp_enqueue_style( $this->plugin_slug . '_admin_styles', plugins_url( 'assets/css/mimo-maps-admin.min.css', __FILE__ ), array( 'dashicons' ), Mimo_Maps_Display::VERSION );
		}
	...
	public function enqueue_admin_scripts() {
		...
		if ( $this->plugin_screen_hook_suffix == $screen->id ) {
			wp_enqueue_script( $this->plugin_slug . '_admin_script', plugins_url( 'assets/js/mimo-maps-admin.min.js', __FILE__ ), array( 'jquery', 'jquery-ui-tabs' ), Mimo_Maps_Display::VERSION );
		}
	...
	public function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix = add_options_page(
				__( 'Mimo Maps Settings', 'mimo-maps' ), $this->plugin_name, 'manage_options', 'mimo-maps-settings', array( $this, 'display_plugin_admin_page' )
		);
		/*
		 * Settings page in the menu
		 * 
		 */
		//$this->plugin_screen_hook_suffix = add_menu_page( __( 'Mimo Maps Settings', 'mimo-maps' ), $this->plugin_name, 'manage_options', 'mimo-maps-settings', array( $this, 'display_plugin_admin_page' ), 'dashicons-location', 81);
	}
	...
	public static function get_term_options(  ) {
	    ...
	    $args1 = wp_parse_args( $args1, array( 'taxonomy' => 'category' ) );

	    

	    $terms1 = (array) get_terms( 'category', $args1 );
	 ...
	 * @since    1.0.0
	 */
	public function mimo_maps_metaboxes() {
		// Start with an underscore to hide fields from custom fields list
		
		


		$mimo_maps_location_metabox = new_cmb2_box( array(
		    
		    'id' => 'mimo_maps_location_box',
		    'title' => __( 'Location' , 'mimo-maps' ),
		    ...
		    $mimo_maps_location_metabox->add_field( array(
		    'name' => 'Location/Route',
		   ...
		   $mimo_maps_location_metabox->add_field( array(
			...
			$mimo_maps_location_ids_array=array();
			$mimo_maps_location_names_array=array();
			...
			while ($my_query->have_posts()) : $my_query->the_post(); 
			    $mimo_maps_location_ids_array[]= get_the_id()  ;
			    $mimo_maps_location_names_array[]= get_the_title( get_the_ID() ) ;
			endwhile;
			endif;
			$mimo_maps_locations_array = array_combine($mimo_maps_location_ids_array, $mimo_maps_location_names_array);
	...
	function settings_import() {
		...
		if ( $extension != 'json' ) {
			wp_die( __( 'Please upload a valid .json file','mimo-maps') );
		}

		$import_file = $_FILES[ 'pn_import_file' ][ 'tmp_name' ];

		if ( empty( $import_file ) ) {
			wp_die( __( 'Please upload a file to import', 'mimo-maps' ) );
		}
		...
		wp_safe_redirect( admin_url( 'options-general.php?page=mimo-maps-settings') );
		exit;
	}
	...
	public function contextual_help() {
		...
		WP_Contextual_Help::register_tab( 'mimo-maps-help-new-post', __( 'Mimo Maps Management', 'mimo-maps' ), array(
		...
		WP_Contextual_Help::register_tab( 'mimo_maps_settings', __( 'Mimo Maps Settings', 'mimo-maps' ), array(
	...
	function custom_initial_pointers( $pointers, $prefix ) {
		return array_merge( $pointers, array(
			$prefix . '_contextual_tab' => array(
				...
				'title' => __( 'Mimo Maps Help', 'mimo-maps' ),
				'text' => __( 'A pointer for help tab.<br>Go to Posts, Pages or Users for other pointers.', 'mimo-maps' ),
				'edge' => 'top',
				'align' => 'right',
